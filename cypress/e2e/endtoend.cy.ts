...
it('pokus_1', function() {
    ...
    // Wait for the saved designs to load
    cy.get('[style="background-color: white; padding: 20px; border-radius: 5px; max-width: 80%; max-height: 80%; overflow: auto;"]')
      .find('button')
      .should('have.length.gt', 0);
  
    // Click on the last (most recent) saved design
    cy.get('[style="background-color: white; padding: 20px; border-radius: 5px; max-width: 80%; max-height: 80%; overflow: auto;"]')
      .find('button')
      .last()
      .click();
    ...
    // Wait for the saved designs to load again
    cy.get('[style="background-color: white; padding: 20px; border-radius: 5px; max-width: 80%; max-height: 80%; overflow: auto;"]')
      .find('button')
      .should('have.length.gt', 0);
  
    // Click on the last (most recent) saved design again
    cy.get('[style="background-color: white; padding: 20px; border-radius: 5px; max-width: 80%; max-height: 80%; overflow: auto;"]')
      .find('button')
      .last()
      .click();
